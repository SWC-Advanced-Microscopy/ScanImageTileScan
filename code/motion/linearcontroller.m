classdef (Abstract) linearcontroller < handle
%%  linearcontroller 
%
% The linearcontroller abstract class is a software entity that represents the 
% physical linear stage controller object. 
%
% The linearcontroller abstract class declares methods that are used to move linear actuators
% A small number of methods are defined within this abstract class. Most methods are defined 
% in concrete classes that must inherit linearcontroller. Examples of these include C891 and BSC201. 
%
%
    ...
    methods
        function obj = attachLinearStage(obj,linearStageObject)
            ...
            if ~regexp(linearStageObject.axisName,'^[xyz]Axis$')
                error('Field stage.settings.axisName is incorrect. It should be one of: xAxis or yAxis. You supplied %s',linearStageObject.axisName)
            end
        ...
        % success - true or false depending on whether a stage is present
            ...
            if isempty(obj.attachedStage)
                disp('No stage object attached. The attachedStage property is empty.')
                return
            end
        ...
        function ready = isAxisReady(obj)
          ...
          if ~obj.isControllerConnected || ~obj.isStageConnected 
            disp('Controller or stages not connected.')
            return
          end
        ...
        function inBounds = isMoveInBounds(obj,targetPosition)
            ...
            if nargin<2
                disp('no targetPosition defined for checking if move is in bounds')
                return
            end
            ...
            if isempty(obj.getMaxPos) || isempty(obj.getMinPos)
                disp('Bounds not set for axis. Not moving.')
                inBounds=false;
                return
            end
        ...
        function success=checkDistanceToMove(obj,thisValue)
            ...
            if isempty(thisValue) 
                disp('Desired move location is empty')
                return
            end            
            if ~isscalar(thisValue)                
                disp('Desired move location must be a scalar')
