...
% C891 is a class that inherits defines the interface to PI's GCS controller class.
%
    ...
    methods
      ...
      function success = isControllerConnected(obj)
        ...
        try 
          ...
          disp('Failed to communicate with C-891 controller')
        end
      ...
      function success=setPositionUnits(obj,controllerUnits,~)
        ...
        if ~strcmp(controllerUnits,'mm')
          disp('C891 units work only in mm')
          success=false;
        end
        success=true;
      ...
      function success=enableAxis(obj)
        ...
        if ~success
          disp('Failed to commincate with the C891 controller to enable axis')
          return
        end

        %Check the enable state
        if obj.hC.qEAX('1')==0
          disp('C891 motor enable state remains off')
          success=false;
          return
        end
        ...
        if obj.hC.qSVO('1')==0
          disp('C891 servo state remains off')
          success=false;
          return
        end
      ...
      function success=disableAxis(obj)
        ...
        if ~success
          disp('Failed to commincate with the C891 controller to disable axis')
          return
        end

        %Check the enable state
        if obj.hC.qEAX('1')==1
          disp('C891 motor enable state remains on')
          success=false;
          return
        end
        ...
        %Check the servo state
        if obj.hC.qSVO('1')==1
          disp('C891 servo state remains on')
          success=false;
          return
        end
