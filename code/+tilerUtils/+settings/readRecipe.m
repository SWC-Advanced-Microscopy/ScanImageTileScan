function [thisRecipe,msg] = readRecipe(recipeFname)
    ...
    %
    % function thisRecipe = tilerUtils.settings.readRecipe(recipeFname)
    %
    % Purpose
    % Read a recipe YAML file and return as a structure. 
    ...
    if ~ischar(recipeFname)
        msg=sprintf('tilerUtils.settings.readRecipe requires a string as an input argument\n');
        fprintf(msg)
        return
    end

    if ~exist(recipeFname,'file')
        msg=sprintf('tilerUtils.settings.readRecipe tried to load a non-existent file at %s\n',recipeFname);
        fprintf(msg)
        return
    end

    try
        tRecipe = tilerUtils.yaml.ReadYaml(recipeFname);
    catch
        L=lasterror;
        fprintf(L.message)
    ...
    for tF = theseFields'
        ...
        if ~isfield(tRecipe,tF{1})
            msg=sprintf('tilerUtils.settings.readRecipe finds no field "%s" in %s. NOT A RECIPE FILE\n',tF{1},recipeFname);
            fprintf(msg);
            return
        end % if ~isfield
        ...
        for sF = subFields'
            ...
            if ~isfield(tRecipe.(tF{1}), sF{1})
                msg=sprintf('tilerUtils.settings.readRecipe finds no field "%s.%s" in %s. NOT A RECIPE FILE\n',tF{1},sF{1},recipeFname);
                fprintf(msg);
                return
            end %if ~isfield
            ...
            if ~isa(recipeVal, class(defaultVal))
                msg=sprintf('tilerUtils.settings.readRecipe expects "%s.%s" to be a %s but it is a %s. USING DEFAULT VALUES.\n', ...
